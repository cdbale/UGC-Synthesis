...
pcd <- as.data.frame(PCA(Xs, number_components = 10))
...
r_state <- pMSE_for_sequence(origdata, Y_syn_conf, Zs, pcd, 'm')
...
# pMSE ratios for state only synthesis using model with main effects
r_state_c <- pMSE_for_sequence(origdata, CART_Y_conf[b_cp_i], Zs, pcd, 'm')
...
for (sim in 1:nsim){
        ...
        # pMSE ratios for state only synthesis using model with main effects
        r_state_c <- pMSE_for_sequence(origdata, CART_Y_conf[b_cp_i], Zs, rp, 'm')
        r_state_c_avg <- sapply(r_state_c, mean)
