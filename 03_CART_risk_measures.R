...
### This program calculates the privacy risk for the CART synthetic data ####
...
for (P in 1:length(Folders)){  ## Large Loop, 
  ...
  if (P==2){I=m;len=length(CART_CP)}            ## Synthetic State AND Rating
  if (P==3){I=m;len=length(CART_CP)}            ## Synthetic State Only
  ...
  for (l in 1:len){
    ...
    colnames(trueprobs)=rep(c("Name","FirstStage","Rank_First","SecondStage","Rank_Second","N","Complexity_Param","Synthesis"),1)
    ...
    for (t in 1:T){  
      ...
      for (i in 1:I){
        ...
        if (P==2){protected$Rating=CART_Z_Y[[l]][[i]][,1];protected$State=reference[CART_Z_Y[[l]][[i]][,2],2]}
        if (P==3){protected$Rating=CART_Y[[l]][[i]][,1];protected$State=reference[CART_Y[[l]][[i]][,2],2]}
        if (P==4){}
        if (P==5){}
      ...
      pr_text=pr=matrix(0,nrow=dim(data2)[1],ncol=I)  ##  matrix of 1881 rows by I simulations to store probabilities
      ...
      trueprobs[t,7]=CART_CP[l]          ## save the lambda value from the synthetic data
    ...
    ## cycle through l here, each value of CP
    trueprobs[,2:7]=(round(trueprobs[,2:7],8))
...
write.csv(probability_results,paste("output/","CART ",today,"/CART_probability_results",m,"syntheses.csv",sep=""),row.names=FALSE)
...
pdf(paste("output/","CART ",today,"/CART_SecondStageProbabilities_",m,"syntheses",".pdf",sep=""))
...
Name_CP_prob_none <- probability_results %>% 
  filter(Synthesis=="None") %>%
  group_by(Name, Complexity_Param, Synthesis) %>%
  ungroup() %>%
  mutate(Complexity_Param=factor(Complexity_Param, 
                                 levels=c(paste(CART_CP[1]),paste(CART_CP[2]),paste(CART_CP[3])),
                                 labels = c(paste(CART_CP_round[1]),paste(CART_CP_round[2]),paste(CART_CP_round[3]))))

Name_CP_prob_none$Confidential <- Name_CP_prob_none$SecondStage


Name_CP_prob <- probability_results %>% 
...
#                                                                       "lambda = 0.03",
#                                                                       "lambda = 0.5"))
s<-ggplot(Name_CP_prob, aes(SecondStage, Name)) +
  #geom_line(aes(group = Name)) +
  geom_point(data = Name_CP_prob_none, aes(size = "Confidential"),
             shape = 8, color = "#009E73") +
  geom_point(aes(color = Complexity_Param), size = 2) +
...
sr<-ggplot(Name_CP_prob, aes(SecondStage, Name)) +
  #geom_line(aes(group = Name)) +
  geom_point(data = Name_CP_prob_none, aes(size = "Confidential"),
             shape = 8, color = "#009E73") + 
  scale_colour_manual(values=cbPalette) + 
  theme_bw(base_family = "") +
  theme(legend.title=element_blank()) +
  geom_point(aes(color = Complexity_Param),size=2) + xlim(0,.12) +
...
pdf(paste("output/","CART ",today,"/CART_Rank_Second_",m,"syntheses",".pdf",sep=""))
...
s<-ggplot(Name_CP_prob, aes(Rank_Second, Name)) +
  #geom_line(aes(group = Name)) + 
  xlim(0,821) +
  geom_point(aes(color = Complexity_Param),size=2) + 
...
sr<-ggplot(Name_CP_prob, aes(Rank_Second, Name)) +
  #geom_line(aes(group = Name)) +
  geom_point(aes(color = Complexity_Param),size=2) + xlim(0,821) +
...
n<-ggplot(Name_CP_prob, aes(Rank_Second, Name)) +
  #geom_line(aes(group = Name)) +
  geom_point(aes(color = Complexity_Param),size=2) + xlim(0,821) +
