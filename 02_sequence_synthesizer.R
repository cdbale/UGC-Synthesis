...
r_X <- add_variable(new_var = Z, new_var_name = "Rating", old_data = X)
...
rating_fit_time <- system.time(rating_model <- glmnet(x = X, 
                                                      y = factor(Z), 
                                                      family="multinomial", 
...
t1 <- system.time(synprob_rating <- predict(rating_model, newx = X, s = sml, type="response"))[3]
...
draw_state_priv <- function(){
  ...
  for (l in 1:length(sml)){
    t1 <- system.time(synprob_state_priv <- lapply(1:m, function(x) predict(state_model, newx=add_variable(Z_syn[[l]][,x], "Rating", X), s=sml[l], type="response")[,,1]))[3]
...
write.csv(b_times, paste0("output/", today, "/proposed_model_times.csv"), row.names = FALSE)
