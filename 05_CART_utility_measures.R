...
prev_cart=topicwords_p_cart=avg_corr_cart=list()
...
# stm <- stm(documents = mm$documents, vocab = mm$vocab, K = K, prevalence=~Rating+State,content=~State,max.em.its = iter,data=meta, init.type = "Spectral")
...
for (P in 2:3){
  avg_corr_cart[[P-1]]=list()
  topicwords_p_cart[[P-1]]=list()
  prev_cart[[P-1]]=list()   #prevalences of Rating covariate
  
  print(P)
  for (i in 1:length(CART_CP)){
    l=CART_CP[i]
    print(CART_CP[i]);print("CP")
    
    avg_corr_cart[[P-1]][[i]]=list()
    topicwords_p_cart[[P-1]][[i]]=list()
    prev_cart[[P-1]][[i]]=list()
    
    for (ii in 1:m){
      print(ii);print("sim")
      
      if (P==2){data_p=CART_Z_Y[[i]][[ii]]}
      if (P==3){data_p=CART_Y[[i]][[ii]]}
      ...
      prev_cart[[P-1]][[i]][[ii]]=matrix(0,nrow=K,ncol=4)
      colnames( prev_cart[[P-1]][[i]][[ii]])=c("High Rating Prevalence","Low Rating Coefficient","p-value","Square Std Error")
      ...
      for (k in 1:K){
        ...
        prev_cart[[P-1]][[i]][[ii]][k,]=c(prev_high_p,prev_low_p,prev_pvalue_p,prev_low_Var_p)
      ...
      avg_corr_cart[[P-1]][[i]][[ii]]=corrs
      topicwords_p_cart[[P-1]][[i]][[ii]]=labelTopics(stm_p)$topics[,1:3]
...
for (P in 2:3){
  asu_cart[[P-1]]=as.data.frame(matrix(0,nrow=K*(length(CART_CP)+1),ncol=4))  ## analysis-specific utility
  colnames(asu_cart[[P-1]])=c("ASU","Comp_Param","Synthesis","Topic")
  
  top_avg=matrix(0,nrow=K,ncol=length(CART_CP))
  prev_cp=list()
  ...
  for (i in 1:length(CART_CP)){
    ...
    for (ii in 1:m){
      ...
      temp=prev_cart[[P-1]][[i]][[ii]]   ## prevalences and p-value
    ...
    for (k in 1:K){
      asu_cart[[P-1]][(k-1)*(length(CART_CP)+1)+i,]=c(colMeans(t(sapply(avg_corr_cart[[P-1]][[i]],colMeans)))[k],CART_CP[i],Folders[P],combine_words(topicwords[k,],and=""))    ## average over the levels of state and M 
      asu_cart[[P-1]][(k)*(length(CART_CP)+1),]=c(1,0,Folders[1],combine_words(topicwords[k,],and=""))    ## average over the levels of state and M  
      ...
      for (top in 1:3){  
        run_total=0
        for (ii in 1:m){
          temp=sum(str_count(topicwords_p_cart[[P-1]][[i]][[ii]][k,],topicwords[k,top]))  ## count the number of times in the top 3
      ...
      colnames(top_avg)=paste(CART_CP,"_CP",sep="")
    }
    
    write.csv(prev_cp[[i]],paste("output/CART ",today,"/Prevalence_",m,"CART_syntheses_",Folders[P],CART_CP[i],"_MB",".csv",sep=""))
    
  }
  
  write.csv(top_avg,paste("output/CART ",today,"/Top3_TopicWords_",m,"CART_syntheses_",Folders[P],".csv",sep=""))
  
  
}


#write.csv(topicwords,paste("output/CART ",today,"/CART_Top3_TopicWords.csv",sep=""))

P=2
asu_results_cart=rbind(asu_cart[[P-1]],asu_cart[[P]])
asu_results_cart$ASU=as.numeric(asu_results_cart$ASU)


write.csv(asu_results_cart,paste("output/CART ",today,"/AnalysisSpecificUtility_",m,"CART_syntheses",".csv",sep=""))

pdf(paste("output/CART ",today,"/AnalysisSpecificUtility_",m,"CART_syntheses",".pdf",sep=""))

# The palette with grey:
cbPalette <- c("#E69F00", "#56B4E9", "#999999", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

temp_level_cart=factor(asu_results_cart$Topic,levels=levels(factor(asu_results_cart$Topic))[c(4,1,2,5,3)])

asu_results_cart$Topics=temp_level_cart

asu_results_cart$Comp_Param <- as.character(round(as.numeric(asu_results_cart$Comp_Param), 8))
CART_CP <- round(CART_CP, 8)
#CART_CP_round <- c(CART_CP[1:2], round(CART_CP[3], 4))
CART_CP_round <- c(CART_CP[1], round(CART_CP[2], 8), round(CART_CP[3], 4))

Name_CP_prob_cart <- asu_results_cart %>% 
  filter(Synthesis=="State") %>%
  group_by(Comp_Param, Synthesis,Topic) %>%
  ungroup() %>%
  mutate(Comp_Param=factor(Comp_Param,levels(factor(asu_results_cart$Comp_Param))[c(1,4,2,3)]),Topic=factor(Topic,levels(factor(asu_results_cart$Topic))[c(4,1,2,5,3)]))

Name_CP_prob_cart_none <- asu_results_cart %>% 
  filter(Synthesis=="None") %>%
  group_by(Comp_Param, Synthesis,Topic) %>%
  ungroup() %>%
  mutate(Comp_Param=factor(Comp_Param,levels(factor(asu_results_cart$Comp_Param))[c(1,4,2,3)]),Topic=factor(Topic,levels(factor(asu_results_cart$Topic))[c(4,1,2,5,3)]))

Name_CP_prob_cart_none$Confidential <- Name_CP_prob_cart_none$ASU


s_cart<-ggplot(Name_CP_prob_cart, aes(ASU, Topic)) +
  #geom_line(aes(group = Topic)) +
  geom_point(data = Name_CP_prob_cart_none, aes(size = "Confidential"),
             shape = 8, color = "#009E73") + 
  scale_colour_manual(values=cbPalette) + 
  theme_bw(base_family = "") +
  theme(legend.title=element_blank()) +
  geom_point(aes(color = factor(Comp_Param, 
                                levels=c(paste(CART_CP[1]),paste(CART_CP[2]),paste(CART_CP[3])),
                                labels = c(paste(CART_CP_round[1]),paste(CART_CP_round[2]),paste(CART_CP_round[3])))),size=2) + 
  xlim(.4,1) +
  labs(title = "State Only",
       x="Average Correlation")

Name_MB_prob_cart <- asu_results_cart %>% 
  filter(Synthesis=="State and Rating") %>%
  group_by(Comp_Param, Synthesis) %>%
  ungroup() %>%
  mutate(Comp_Param=factor(Comp_Param,levels(factor(asu_results_cart$Comp_Param))[c(1,4,2,3)]),Topic=factor(Topic,levels(factor(asu_results_cart$Topic))[c(4,1,2,5,3)]))


sr_cart<-ggplot(Name_CP_prob_cart, aes(ASU, Topic)) +
  #geom_line(aes(group = Topic)) +
  geom_point(data = Name_CP_prob_cart_none, aes(size = "Confidential"),
             shape = 8, color = "#009E73") + 
  scale_colour_manual(values=cbPalette) + 
  ...
  geom_point(aes(color = factor(Comp_Param, 
                                levels = c(paste(CART_CP[1]),paste(CART_CP[2]),paste(CART_CP[3])),
                                labels = c(paste(CART_CP_round[1]),paste(CART_CP_round[2]),paste(CART_CP_round[3])))), size=2) + xlim(.4,1) +
  labs(title = "State and Rating",x="Average Correlation")

#grid.arrange(s_cart, sr_cart, nrow = 2)
#grid.arrange(sr_cart, nrow = 2)
grid.arrange(s_cart, nrow = 2)
dev.off()
